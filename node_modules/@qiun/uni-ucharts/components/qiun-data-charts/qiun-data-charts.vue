...
export default {
  ...
  mounted() {
    ...
    this.inH5 = true;
    {
      // mounted 中不可访问 created 内的 const systemInfo，需重新获取
      const si = uni.getSystemInfoSync()
      const pl = String(si.platform || '').toLowerCase()
      // 原逻辑仅 Windows/Mac 开启鼠标拖拽；Linux 等桌面浏览器无法横向滚动图表
      const h5DesktopLike = pl !== 'ios' && pl !== 'android'
      if (this.inWin === true || h5DesktopLike) {
